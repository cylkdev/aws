if Code.ensure_loaded?(SandboxRegistry) do
  defmodule AWS.IdentityCenter.Sandbox do
    ...
    def provision_permission_set_response(permission_set_arn, opts) do
      doc_examples = ["fn -> ...", "fn (opts) -> ..."]
      func = find!(:provision_permission_set, permission_set_arn, doc_examples)
    ...
    def update_identity_store_user_response(user_id, opts) do
      doc_examples = ["fn -> ...", "fn (opts) -> ..."]
      func = find!(:update_identity_store_user, user_id, doc_examples)
    ...
    def describe_identity_store_user_response(user_id, opts) do
      doc_examples = ["fn -> ...", "fn (opts) -> ..."]
      func = find!(:describe_identity_store_user, user_id, doc_examples)
    ...
    def describe_identity_store_group_response(group_id, opts) do
      doc_examples = ["fn -> ...", "fn (opts) -> ..."]
      func = find!(:describe_identity_store_group, group_id, doc_examples)
    ...
    def set_list_instances_responses(funcs),
      do: set_responses(:list_instances, Enum.map(funcs, &{"*", &1}))

    def set_create_permission_set_responses(tuples),
      do: set_responses(:create_permission_set, tuples)

    def set_delete_permission_set_responses(tuples),
      do: set_responses(:delete_permission_set, tuples)

    def set_list_permission_sets_responses(tuples),
      do: set_responses(:list_permission_sets, tuples)

    def set_attach_managed_policy_to_permission_set_responses(tuples),
      do: set_responses(:attach_managed_policy_to_permission_set, tuples)

    def set_detach_managed_policy_from_permission_set_responses(tuples),
      do: set_responses(:detach_managed_policy_from_permission_set, tuples)

    def set_create_account_assignment_responses(tuples),
      do: set_responses(:create_account_assignment, tuples)

    def set_delete_account_assignment_responses(tuples),
      do: set_responses(:delete_account_assignment, tuples)

    def set_provision_permission_set_responses(tuples),
      do: set_responses(:provision_permission_set, tuples)

    def set_create_identity_store_user_responses(tuples),
      do: set_responses(:create_identity_store_user, tuples)

    def set_delete_identity_store_user_responses(tuples),
      do: set_responses(:delete_identity_store_user, tuples)

    def set_update_identity_store_user_responses(tuples),
      do: set_responses(:update_identity_store_user, tuples)

    def set_describe_identity_store_user_responses(tuples),
      do: set_responses(:describe_identity_store_user, tuples)

    def set_describe_identity_store_group_responses(tuples),
      do: set_responses(:describe_identity_store_group, tuples)

    def set_list_identity_store_users_responses(tuples),
      do: set_responses(:list_identity_store_users, tuples)

    def set_create_identity_store_group_responses(tuples),
      do: set_responses(:create_identity_store_group, tuples)

    def set_delete_identity_store_group_responses(tuples),
      do: set_responses(:delete_identity_store_group, tuples)

    def set_list_identity_store_groups_responses(tuples),
      do: set_responses(:list_identity_store_groups, tuples)

    def set_create_group_membership_responses(tuples),
      do: set_responses(:create_group_membership, tuples)

    def set_delete_group_membership_responses(tuples),
      do: set_responses(:delete_group_membership, tuples)
