if Code.ensure_loaded?(SandboxRegistry) do
  defmodule AWS.EventBridge.Sandbox do
    ...
    def set_list_rules_responses(funcs),
      do: set_responses(:list_rules, Enum.map(funcs, fn f -> {"*", f} end))
    ...
    def set_list_targets_by_rule_responses(tuples),
      do: set_responses(:list_targets_by_rule, tuples)
    ...
    def set_list_connections_responses(funcs),
      do: set_responses(:list_connections, Enum.map(funcs, fn f -> {"*", f} end))

    def set_create_api_destination_responses(tuples),
      do: set_responses(:create_api_destination, tuples)

    def set_describe_api_destination_responses(tuples),
      do: set_responses(:describe_api_destination, tuples)

    def set_update_api_destination_responses(tuples),
      do: set_responses(:update_api_destination, tuples)

    def set_delete_api_destination_responses(tuples),
      do: set_responses(:delete_api_destination, tuples)

    def set_list_api_destinations_responses(funcs),
      do: set_responses(:list_api_destinations, Enum.map(funcs, fn f -> {"*", f} end))
    ...
    def set_list_event_buses_responses(funcs),
      do: set_responses(:list_event_buses, Enum.map(funcs, fn f -> {"*", f} end))

    def set_put_events_responses(funcs),
      do: set_responses(:put_events, Enum.map(funcs, fn f -> {"*", f} end))
