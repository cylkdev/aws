defmodule Mix.Tasks.Aws.Iam.SetupRole do
  ...
  def run(argv) do
    ...
    role_opts =
      opts
      |> Helpers.maybe_put(:description, parsed[:role_description])

    policy_opts =
      opts
      |> Helpers.maybe_put(:description, parsed[:policy_description])

    # Step 1: Create role
    Mix.shell().info("Step 1/3: Creating role '#{role_name}'...")
    role_result = Helpers.idempotent(
      role_name,
    ...
    attach_result = Helpers.idempotent(
      "#{role_name}/#{policy_name}",
      fn ->
        case AWS.IAM.list_attached_role_policies(role_name, opts) do
          {:ok, %{policies: policies}} ->
            if Enum.any?(policies, &(&1[:policy_arn] == policy_arn)),
              do: {:ok, %{}},
