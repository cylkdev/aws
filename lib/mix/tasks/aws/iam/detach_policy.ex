  ...
  def run(argv) do
    ...
    case {parsed[:from_role], parsed[:from_user], parsed[:from_group]} do
      {role, nil, nil} when is_binary(role) ->
        AWS.IAM.detach_role_policy(role, policy_arn, opts) |> Helpers.handle_result()

      {nil, user, nil} when is_binary(user) ->
        AWS.IAM.detach_user_policy(user, policy_arn, opts) |> Helpers.handle_result()

      {nil, nil, group} when is_binary(group) ->
        AWS.IAM.detach_group_policy(group, policy_arn, opts) |> Helpers.handle_result()
