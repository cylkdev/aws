defmodule Mix.Tasks.Aws.S3.SetupEvents do
  ...
  def run(argv) do
    ...
    result =
      Helpers.idempotent(
        ...
        fn ->
          AWS.EventBridge.put_rule(rule, Keyword.merge(opts, event_pattern: event_pattern, state: state))
          |> Helpers.handle_result()
        end,
        force
      )
  ...
  defp build_event_pattern("created", bucket), do: AWS.EventBridge.s3_object_created_pattern(bucket)
  defp build_event_pattern("deleted", bucket), do: AWS.EventBridge.s3_object_deleted_pattern(bucket)
