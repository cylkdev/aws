defmodule Mix.Tasks.Aws.Eventbridge.CreateConnection do
  ...
  def run(argv) do
    ...
    create_opts = opts |> Helpers.maybe_put(:description, parsed[:description])

    Helpers.idempotent(
      name,
      fn -> AWS.EventBridge.describe_connection(name, opts) end,
      fn ->
        AWS.EventBridge.create_connection(name, authorization_type, auth_parameters, create_opts)
  ...
  defp build_auth("api_key", parsed) do
    ...
    key_value = parsed[:api_key_value] || Mix.raise("--api-key-value is required for api_key auth")
  ...
  defp build_auth("oauth", parsed) do
    ...
    client_secret = parsed[:client_secret] || Mix.raise("--client-secret is required for oauth auth")
    endpoint = parsed[:authorization_endpoint] || Mix.raise("--authorization-endpoint is required for oauth auth")
  ...
  defp build_auth(type, _), do: Mix.raise("Unknown auth type '#{type}'. Use: api_key, basic, oauth")
end
