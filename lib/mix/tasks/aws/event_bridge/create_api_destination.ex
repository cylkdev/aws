defmodule Mix.Tasks.Aws.Eventbridge.CreateApiDestination do
  ...
  def run(argv) do
    ...
    [name, connection_arn, url, http_method] =
      case args do
        [n, c, u, m | _] -> [n, c, u, m]
        _ -> Mix.raise("Usage: mix aws.eventbridge.create_api_destination NAME CONNECTION_ARN URL HTTP_METHOD")
      end
    ...
    Helpers.idempotent(
      ...
      fn ->
        AWS.EventBridge.create_api_destination(name, connection_arn, url, http_method, dest_opts)
