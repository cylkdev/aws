defmodule Mix.Tasks.Aws.IdentityCenter.SetupAccountAccess do
  ...
  def run(argv) do
    ...
    permission_set_name = parsed[:permission_set_name] || Mix.raise("--permission-set-name is required")
    managed_policy_arn = parsed[:managed_policy_arn] || Mix.raise("--managed-policy-arn is required")
    ...
    ps_result = Helpers.idempotent(
      permission_set_name,
      fn -> {:error, %{code: :not_found}} end,
      fn -> AWS.IdentityCenter.create_permission_set(instance_arn, permission_set_name, ps_opts) end,
      force
    )
    if match?({:error, _}, ps_result), do: Helpers.handle_result(ps_result)

    permission_set_arn = case ps_result do
    ...
    attach_result = AWS.IdentityCenter.attach_managed_policy_to_permission_set(
      instance_arn,
      permission_set_arn,
      managed_policy_arn,
      opts
